...
impl std::fmt::Display for BumpSpec {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            ...
            BumpSpec::Specific(_m, _mi, _p) => "specific",
        })
    }
}
...
pub(crate) fn bump_package_with_spec(
    ...
    let package_version_must_be_breaking = match bump_spec {
        Major | Minor | Patch | Specific(..) => bump_major_minor_patch(&mut v, bump_spec),
        Keep => false,
        Auto => {
            let segments = crate::git::history::crate_ref_segments(
                ...
                ctx.history
                    .as_ref()
                    .expect("BUG: assure history is set here"),
    ...
    let (latest_release, next_release) = match ctx.crates_index.crate_(&package.name) {
        Some(published_crate) => {
            let latest_release =
                semver::Version::parse(published_crate.most_recent_version().version())
            ...
            let next_release = if latest_release >= desired_release {
                ...
                if bump_when_needed
                    && package.version > latest_release
                    && desired_release != package.version
                {
